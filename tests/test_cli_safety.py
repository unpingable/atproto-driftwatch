...
def test_live_with_confirm_and_cap(tmp_path, monkeypatch):
    monkeypatch.setenv("LABELER_EMIT_MODE", "emit")
    monkeypatch.setenv("LABELER_EMIT_CONFIRM", "true")
    in_path = Path("fixtures/posts.jsonl")
    out_path = tmp_path / "labels_out.jsonl"
    ...
    assert _P("out/live_emits.jsonl").exists()
