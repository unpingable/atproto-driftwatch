...
def _sqlite(db_path: Path, now: datetime) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        conn.rollback()
        conn.close()
        facts.update({"quick_check": quick, "write_transaction_acquired": True, "page_size": page_size, "page_count": page_count, "freelist_count": freelist, "auto_vacuum": auto_vacuum})
        continuity = _obs("PRESENT" if quick == "ok" else "DEGRADED", now.isoformat(), None, "SQLite is readable and can acquire a write transaction" if quick == "ok" else "SQLite quick_check did not return ok", facts)
        ...
        return _obs("DEGRADED", now.isoformat(), None, "SQLite continuity probe failed", facts), _obs("UNKNOWN", now.isoformat(), None, "SQLite internal slack is unobservable", facts)
