...
| `driftwatch.persistence.sqlite_continuity` | The configured DB is readable, `quick_check(1)` is `ok`, and a non-mutating `BEGIN IMMEDIATE` write transaction can be acquired. | The file is absent, unreadable, corrupt, or cannot acquire write intent. This is narrower than “database healthy.” |
